      ...
      <div class="flex items-center justify-between mb-6">
        <div>
          ...
          <p class="text-gray-500 dark:text-gray-400 text-sm">
            {{ platos().length }} platos en la carta
          </p>
        </div>
          ...
          <div class="card overflow-hidden group">
            <div class="relative h-40 overflow-hidden">
              ...
              >
                <button
                  (click)="editar(p)"
                  class="w-9 h-9 rounded-lg bg-white text-cyan-700 flex items-center justify-center hover:bg-cyan-50"
                >
                  ✏️
                </button>
                <button
                  (click)="eliminar(p.id_plato)"
                  class="w-9 h-9 rounded-lg bg-white text-red-600 flex items-center justify-center hover:bg-red-50"
                >
                  🗑️
                </button>
              </div>
            ...
            <div class="p-3">
              ...
              <p class="font-bold text-cyan-700 dark:text-cyan-400 text-sm">
        ...
        <div class="card w-full max-w-lg p-6" (click)="$event.stopPropagation()">
          ...
          <form (ngSubmit)="guardar()" class="space-y-4">
            ...
            <div class="flex gap-3 pt-2">
              <button type="button" (click)="cerrarModal()" class="btn-outline flex-1">
                Cancelar
              </button>
...
export class PlatosAdminComponent implements OnInit {
  ...
  guardar(): void {
    ...
    let obs = null;
    if (this.editando()) {
      obs = this.api.actualizarPlato(this.editando().id_plato, fd);
    } else {
      obs = this.api.crearPlato(fd);
    }
  ...
  eliminar(id: number): void {
    ...
    this.api.eliminarPlato(id).subscribe({
      next: () => {
        this.toast.success('Plato eliminado');
        this.cargar();
      },
