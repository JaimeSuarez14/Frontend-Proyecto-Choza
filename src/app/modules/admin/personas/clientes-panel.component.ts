...
@Component({
  ...
  standalone: true,
  imports: [ FormsModule, ClienteModalComponent],
  template: `
    <div class="space-y-8">
      ...
      <div
        class="overflow-x-auto bg-gray-50 dark:bg-gray-950/40 rounded-3xl border border-gray-100 dark:border-gray-800"
      >
        |
        <table class="w-full min-w-175">
          ...
          <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
            @for (c of clientes(); track c.id_cliente) {
              <tr class="hover:bg-indigo-50/50 dark:hover:bg-indigo-950/20 transition-colors">
                <td class="px-6 py-5">
                ...
                <td class="px-6 py-5 text-right">
                  <div class="flex items-center justify-end gap-2.5">
                    <button
                      ...
                      class="p-2.5 rounded-lg bg-indigo-100 dark:bg-indigo-900 text-indigo-700 dark:text-indigo-300 hover:bg-indigo-200 dark:hover:bg-indigo-800 transition-colors"
                    >
                      ✏️
                    </button>
                    <button
                      (click)="eliminarCliente(c.id_cliente)"
                      class="p-2.5 rounded-lg bg-red-100 dark:bg-red-900 text-red-700 dark:text-red-300 hover:bg-red-200 dark:hover:bg-red-800 transition-colors"
                    >
                      🗑️
                    </button>
                  </div>
