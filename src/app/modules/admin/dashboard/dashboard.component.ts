      ...
      @if (stats()) {
        <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          ...
          <div class="card p-5 border-l-4 border-green-500">
            ...
            <p class="text-2xl font-bold text-gray-800 dark:text-gray-100">
              S/ {{ stats()!.kpis.ingresos_totales | number: '1.2-2' }}
            </p>
          ...
          <div class="card p-5 border-l-4 border-yellow-500">
            ...
            <p class="text-2xl font-bold text-gray-800 dark:text-gray-100">
              S/ {{ stats()!.kpis.ingresos_hoy | number: '1.2-2' }}
            </p>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          <a
            ...
            class="card p-4 flex flex-col items-center gap-2 hover:border-cyan-400 transition-colors cursor-pointer text-center hover:scale-105 duration-300 transform"
          >
            ...
            <span class="text-xs text-amber-600 font-semibold"
              >{{ stats()!.kpis.pendientes }} pendientes</span
            >
          </a>
          <a
            routerLink="/admin/platos"
            class="card p-4 flex flex-col items-center gap-2 hover:border-cyan-400 transition-colors cursor-pointer text-center hover:scale-105 duration-300 transform"
          >
                  ...
                  <div class="flex-1 min-w-0">
                    ...
                    <div class="w-full bg-gray-100 dark:bg-gray-700 rounded-full h-1.5 mt-1">
                      <div
                        ...
                        [style.width.%]="
                          (p.cantidad_vendida / stats()!.productos_top[0].cantidad_vendida) * 100
                        "
                      ></div>
                    </div>
                  </div>
                  <span class="text-xs font-semibold text-cyan-700 dark:text-cyan-400 shrink-0"
                    >{{ p.cantidad_vendida }} und.</span
                  >
                </div>
              }
            </div>
          ...
          <div class="card p-5">
            ...
            <div class="space-y-3">
              @for (c of stats()!.clientes_frecuentes; track c.id_cliente) {
                <div class="flex items-center gap-3">
                  <div
                    class="w-9 h-9 rounded-full bg-linear-to-br from-cyan-500 to-cyan-700 text-white flex items-center justify-center text-sm font-bold shrink-0"
                  >
                  ...
                  <span class="text-sm font-bold text-green-600 shrink-0"
                    >S/ {{ c.gasto_total | number: '1.2-2' }}</span
                  >
                </div>
              }
            </div>
...
export class DashboardComponent implements OnInit {
  ...
  // 1. Convertimos 'hoy' en un signal con el formato inicial
  hoy = signal<string>(this.obtenerFechaHora());

  ngOnInit(): void {
    this.api.getEstadisticas().subscribe({ next: (r) => this.stats.set(r) });
    // 2. Creamos un intervalo que actualiza el signal cada segundo
    const interval = setInterval(() => {
      this.hoy.set(this.obtenerFechaHora());
    }, 1000);

    // Registramos la limpieza para cuando el componente se destruya
