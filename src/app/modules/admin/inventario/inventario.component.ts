  ...
  template: ` <div class="space-y-4">
      <div class="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p class="text-sm text-gray-500 dark:text-gray-400">
            Control de insumos y existencias en tiempo real
          </p>
        </div>
        <div class="flex gap-2">
          @if (bajosStock() > 0) {
            <span
              class="bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400 text-xs font-semibold px-3 py-1.5 rounded-full flex items-center gap-1"
            >
              <i class="fa-solid fa-triangle-exclamation"></i> {{ bajosStock() }} bajo stock
            </span>
          }
          <button
            (click)="nuevoModal.set(true)"
            class="btn-primary-choza flex items-center gap-2 text-sm"
          >
              ...
              <tbody>
                @for (item of inventario(); track item.id_inventario) {
                  <tr>
                    ...
                    <td [class.text-red-600]="item.bajo_stock" [class.font-bold]="item.bajo_stock">
                      {{ item.stock_actual }}
                    </td>
                    ...
                    <td>
                      @if (item.bajo_stock) {
                        <span class="badge-cancelado text-xs px-2 py-0.5 rounded-full font-medium"
                          ><i class="fa-solid fa-exclamation-triangle mr-1"></i>Reponer</span
                        >
                      } @else {
                        <span class="badge-entregado text-xs px-2 py-0.5 rounded-full font-medium"
                          ><i class="fa-solid fa-check-circle mr-1"></i>OK</span
                        >
                      }
                    </td>
                    <td>
                      <div class="flex gap-1">
                        ...
                        <button
                          (click)="eliminar(item.id_inventario)"
    ...
    @if (nuevoModal()) {
      <div
        ...
        (click)="nuevoModal.set(false)"
      >
        <div
          class="bg-white dark:bg-gray-800 rounded-2xl p-6 max-w-sm w-full shadow-2xl animate-slide-up"
          (click)="$event.stopPropagation()"
        >
          <h3 class="font-bold text-lg dark:text-white mb-4">Nuevo Insumo</h3>
          <div class="space-y-3">
            <input
              [(ngModel)]="nuevoForm.nombre_insumo"
              class="input-choza"
            ...
            <div class="grid grid-cols-2 gap-2">
              <input
                type="number"
                [(ngModel)]="nuevoForm.stock_actual"
                class="input-choza"
                placeholder="Stock inicial"
              />
              <input
                type="number"
                [(ngModel)]="nuevoForm.stock_minimo"
                class="input-choza"
          ...
          <div class="flex gap-2 mt-4">
            ...
            <button (click)="nuevoModal.set(false)" class="btn-secondary-choza flex-1">
              Cancelar
            </button>
          </div>
        </div>
    ...
    @if (movimientoItem()) {
      ...
      >
        ...
        >
          ...
          <div class="flex gap-2 mt-4">
            <button (click)="guardarMovimiento()" class="btn-primary-choza flex-1">
              Registrar
            </button>
            <button (click)="movimientoItem.set(null)" class="btn-secondary-choza flex-1">
              Cancelar
            </button>
          </div>
        </div>
...
export class AdminInventarioComponent implements OnInit {
  ...
  cargar(): void {
    ...
    this.api.getInventario().subscribe({
      next: (r) => {
        this.inventario.set(r.inventario);
  ...
  guardarNuevo(): void {
    this.api.crearInsumo(this.nuevoForm).subscribe(() => {
      this.nuevoModal.set(false);
      this.cargar();
    });
  }
  guardarMovimiento(): void {
    ...
    this.api.agregarMovimiento(id, this.movForm).subscribe(() => {
      this.movimientoItem.set(null);
      this.cargar();
    });
  }
  eliminar(id: number): void {
    if (!confirm('¿Eliminar este insumo?')) return;
    this.api.eliminarInsumo(id).subscribe(() => this.cargar());
  }
}
