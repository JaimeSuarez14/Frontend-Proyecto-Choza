    ...
    <div class="w-full max-w-md">
      <div class="card-choza rounded-2xl p-8 shadow-lg">
        <div class="text-center mb-6">
          <div class="w-14 h-14 bg-green-700 rounded-full flex items-center justify-center mx-auto mb-3">
        ...
        <div class="space-y-4">
          <div>
            ...
            <input [(ngModel)]="usuario" class="input-choza" placeholder="admin" (keyup.enter)="submit()">
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Contraseña</label>
            <input type="password" [(ngModel)]="password" class="input-choza" placeholder="••••••" (keyup.enter)="submit()">
          </div>
          <button (click)="submit()" [disabled]="loading()"
                  class="btn-primary-choza w-full py-3 flex items-center justify-center gap-2">
            @if (loading()) { <i class="fa-solid fa-spinner animate-spin"></i> Verificando... }
            @else { <i class="fa-solid fa-lock"></i> Acceder al Panel }
          </button>
        </div>
...
export class LoginAdminComponent {
  ...
  submit(): void {
    if (!this.usuario || !this.password) { this.error.set('Completa todos los campos'); return; }
    this.loading.set(true); this.error.set('');
    this.auth.loginAdmin(this.usuario, this.password).subscribe({
      next: () => this.router.navigate(['/admin']),
      error: err => { this.error.set(err.error?.error ?? 'Credenciales incorrectas'); this.loading.set(false); }
    });
  }
}
