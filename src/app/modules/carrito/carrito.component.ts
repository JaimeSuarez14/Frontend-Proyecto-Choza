    ...
    <div class="max-w-4xl mx-auto px-4 py-10">
      ...
      @if (carrito.estaVacio()) {
        <div class="card p-16 text-center fade-in">
          ...
          <h2 class="text-xl font-semibold text-gray-600 dark:text-gray-300 mb-2">
        ...
        <div class="grid lg:grid-cols-3 gap-6">
          ...
          <div class="lg:col-span-2 space-y-4">
            @for (item of carrito.items(); track item.id_plato) {
              <div class="card p-4 flex items-center gap-4 fade-in">
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <p class="text-cyan-700 dark:text-cyan-400 font-bold">
                ...
                <div class="flex items-center gap-2 shrink-0">
                  ...
                  >
                    <button
                      (click)="carrito.actualizar(item.id_plato, item.cantidad - 1)"
                      class="w-8 h-8 flex items-center justify-center hover:bg-gray-100 dark:hover:bg-gray-700 font-bold text-gray-600 dark:text-gray-300"
                    >
                      −
                    </button>
                    <span
                      class="w-10 text-center text-sm font-medium text-gray-800 dark:text-gray-200"
                      >{{ item.cantidad }}</span
                    >
                    <button
                      (click)="carrito.actualizar(item.id_plato, item.cantidad + 1)"
            ...
            <button
              ...
              class="text-sm text-red-500 hover:text-red-700 flex items-center gap-1 mt-2"
            >
              🗑️ Vaciar carrito
            </button>
          </div>
          ...
          <div>
            <div class="card p-6 sticky top-24">
                ...
                @for (item of carrito.items(); track item.id_plato) {
                  <div class="flex justify-between text-gray-600 dark:text-gray-400">
                    <span class="truncate max-w-32"
                      >{{ item.nombre_plato }} x{{ item.cantidad }}</span
                    >
              ...
              <div class="border-t border-gray-200 dark:border-gray-700 pt-3 mb-5">
                <div
                  class="flex justify-between font-bold text-gray-800 dark:text-gray-100 text-lg"
                >
                  <span>Total</span>
              ...
              <a
                routerLink="/checkout"
                class="btn-ocean w-full py-3 block text-center text-base rounded-xl"
              >
                ✅ Proceder al pago
              </a>
              <a
                routerLink="/menu"
                class="btn-outline w-full py-2.5 block text-center text-sm rounded-xl mt-2"
              >
                ← Seguir comprando
              </a>
            </div>
